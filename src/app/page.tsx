...
type Task = { _id?: string; id?: string; title: string; description?: string; priority?: string; workflow_stage_id?: string; due_date?: string | null };
type User = { name?: string; email?: string; role?: string };
type Activity = { userName?: string; message?: string; action?: string; createdAt?: string };
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    api.tasks.list(selectedId).then((result) => setTasks(unwrap(result, [] as unknown[]) as Task[])).catch(() => setTasks([]));
  }, [selectedId]);
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="brand"><span className="brand-mark">P</span><span>pms<span className="brand-dot">.</span></span></div>
        <div className="workspace-label">Workspace <button className="icon-button" aria-label="Workspace menu">⌄</button></div>
        <nav className="nav-list">
          <Link className="nav-item active" href="/"><span>◈</span> Overview</Link>
          <Link className="nav-item" href="/?view=tasks"><span>▣</span> My tasks <b>8</b></Link>
          <Link className="nav-item" href="/?view=calendar"><span>◌</span> Calendar</Link>
          <Link className="nav-item" href="/activity"><span>⌁</span> Activity</Link>
        </nav>
        <div className="project-heading"><span>Projects</span>{user?.role !== "member" && <button className="icon-button" onClick={() => setShowProjectForm(true)} aria-label="Add project">+</button>}</div>
        <div className="project-list">{projects.map((project) => <button className={`project-link ${itemId(project) === selectedId ? "selected" : ""}`} key={itemId(project)} onClick={() => setSelectedId(itemId(project))}><i />{project.name}</button>)}</div>
        <div className="sidebar-footer"><div className="avatar">{user?.name?.slice(0, 2).toUpperCase() ?? "?"}</div><div><strong>{user?.name ?? "Account"}</strong><small>{user?.role ?? "Member"}</small></div><button className="icon-button">•••</button></div>
      </aside>
      <main className="main-content">
        ...
        <section className="content-wrap">
          <div className="page-intro"><div><p className="eyebrow">{new Date().toLocaleDateString(undefined, { weekday: "long", month: "long", day: "numeric", year: "numeric" })}</p><h1>Good morning, {user?.name ?? "there"} <span>✦</span></h1><p className="intro-copy">Here&apos;s the pulse of your workspace.</p></div><button className="primary-button" onClick={() => setShowTaskForm(true)} disabled={!selectedId}><span>+</span> New task</button></div>
          ...
          <div className="section-row"><div><h2>Project focus</h2><p>What&apos;s moving across your projects</p></div><button className="text-button">View all projects <span>→</span></button></div>
