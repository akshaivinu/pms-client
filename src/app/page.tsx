...
export default function Home() {
  ...
  useEffect(() => {
    ...
    api.dashboard
      .recentActivity()
      .then((result) =>
        setActivity(unwrap(result, [] as unknown[]) as Activity[]),
      )
      .catch(() => undefined);
  }, [dispatch, router]);
      ...
      .then((result) =>
        dispatch(
          setProjectTasks({
            ...
            tasks: unwrap(result, [] as unknown[]) as Task[],
          }),
        ),
      )
      .catch(() =>
        dispatch(setProjectTasks({ projectId: selectedId, tasks: [] })),
      );
  }, [dispatch, selectedId]);
  ...
  return (
    <div className="app-shell">
      <Sidebar
        ...
        onSelectProject={(projectId) => dispatch(selectProject(projectId))}
          ...
          <div className="page-intro">
            <div>
              ...
              <p className="intro-copy">
                {showingTasks
                  ? `Tasks in ${selectedProject?.name ?? "your workspace"}.`
          ...
          {!showingTasks && (
            <>
              <div className="section-row">
                <div>
                  <h2>Project focus</h2>
                  <p>What&apos;s moving across your projects</p>
                </div>
                      ...
                      <div>
                        ...
                        <p>
                          {selectedProject?.description ??
                            "Select an accessible project to see its details."}
                        </p>
                      </div>
                    </div>
                    ...
                    <i
                      style={{
                        width: selectedProject
                          ? `${Math.min(100, Math.round((summary.completedTasks / Math.max(summary.totalTasks, 1)) * 100))}%`
                          : "0%",
                      }}
                    />
                  </div>
                    ...
                    {activity.length ? (
                      activity.slice(0, 3).map((event, index) => (
                        <div key={index}>
                          ...
                          <p>
                            <b>{event.userName ?? "Team member"}</b>{" "}
                            {event.message ??
                              event.action ??
                              "updated the workspace"}
                            <small>{event.createdAt ?? "Recently"}</small>
                          </p>
                        </div>
                      ))
                    ) : (
