...
export default function ProjectsPage() {
  ...
  useEffect(() => {
    api.auth
      ...
      .then((result) =>
        setRole(
          String(
            (unwrap(result, {}) as Record<string, unknown>).role ?? "member",
          ),
        ),
      )
      .catch(() => setMessage("Sign in to load your projects."));
    api.projects
      .list()
      .then((result) => {
        const data = unwrap(result, [] as unknown[]) as Project[];
        setProjects(data);
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      setMessage(
        "Could not create the project. Check your session and try again.",
      );
    }
  }
      ...
      <section className="simple-content">
        <div className="simple-title">
          <div>
            ...
            <h1>Projects</h1>
            <p>Every initiative, with room to move.</p>
          </div>
          {role !== "member" && (
            <button
        ...
        <div className="project-grid">
          {projects.map((project) => (
            <Link
              ...
              href={`/projects/${idOf(project)}`}
              key={idOf(project)}
            >
              <span className="project-symbol">{project.name[0]}</span>
              <h2>{project.name}</h2>
              <p>{project.description || "No description yet"}</p>
              <small>
                {project.status ?? "active"} <b>→</b>
              </small>
            </Link>
          ))}
        </div>
